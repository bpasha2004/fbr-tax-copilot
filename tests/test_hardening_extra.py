...
from src.shared.auth import revoke_session
from src.shared.models import init_db, get_engine, advisors
from src.shared.payment_service import create_payment, record_provider_event, reconcile, request_refund, mark_refunded, open_chargeback
...
def test_provider_mutating_request_requires_idempotency(monkeypatch):
    ...
    with pytest.raises(ValueError, match="Idempotency-Key"):
        import asyncio; asyncio.run(client.request("POST", "/payments", {"amount": "1"}))
...
def test_payment_lifecycle_enforces_advisor_ownership():
    ...
    init_db()
    c1, token1 = _login('owner-a@example.com')
